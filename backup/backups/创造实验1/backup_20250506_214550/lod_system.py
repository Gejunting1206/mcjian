...
LOD_LEVELS = [
    {'distance': 4, 'name': '高细节', 'collision': True, 'update_interval': 0.5},  # 近距离 - 高细节
    {'distance': 8, 'name': '中细节', 'collision': True, 'update_interval': 1.0},   # 中距离 - 中细节
    {'distance': 16, 'name': '低细节', 'collision': False, 'update_interval': 2.0}   # 远距离 - 低细节
]
...
class LODManager:
    ...
    def __init__(self):
        ...
        self.update_interval = 0.3  # 增加全局更新间隔以减少CPU负担
        self.stats = {
            'high_detail': 0,
            'medium_detail': 0,
            'low_detail': 0,
    ...
    def update(self):
        ...
        if current_fps < LOW_FPS:
            # 帧率很低，极大幅度减小LOD距离阈值
            adjustment_factor = max(0.4, (current_fps / LOW_FPS))  # 更激进的调整因子
            LOD_LEVELS[0]['distance'] = max(4, int(LOD_LEVELS[0]['distance'] * adjustment_factor))
            LOD_LEVELS[1]['distance'] = max(8, int(LOD_LEVELS[1]['distance'] * adjustment_factor))
            # 增加更新间隔，减少CPU负担
            self.update_interval = min(0.4, self.update_interval * 1.1)
        elif current_fps < TARGET_FPS:
            # 帧率低于目标，适度减小LOD距离阈值
            adjustment_factor = max(0.7, (current_fps / TARGET_FPS))
            LOD_LEVELS[0]['distance'] = max(6, int(LOD_LEVELS[0]['distance'] * adjustment_factor))
            LOD_LEVELS[1]['distance'] = max(12, int(LOD_LEVELS[1]['distance'] * adjustment_factor))
            # 适度增加更新间隔
            self.update_interval = min(0.3, self.update_interval * 1.05)
        elif current_fps > HIGH_FPS:
            # 帧率高，适度增加LOD距离阈值
            LOD_LEVELS[0]['distance'] = min(12, LOD_LEVELS[0]['distance'] + 1)
            LOD_LEVELS[1]['distance'] = min(24, LOD_LEVELS[1]['distance'] + 1)
            # 减少更新间隔，提高响应性
