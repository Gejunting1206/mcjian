...
class ChunkLoadingOptimizer:
    ...
    def __init__(self):
        ...
        self.fps_history = deque(maxlen=20) # 记录最近20帧的FPS (增加样本量)
        self.frame_time_history = deque(maxlen=20) # 记录最近20帧的帧时间 (增加样本量)
    ...
    def update(self, player_position, player_direction=None, delta_time=0.016):
        ...
        chunk_loader.queue_chunks_around_player(player_position, player_direction)
    ...
    def _get_adaptive_chunks_per_frame(self):
        ...
        # 如果帧时间远低于目标，可以尝试大幅增加加载量
        if avg_frame_time < self.target_frame_time * 0.7:
            # 更激进地增加，大幅提高上限
            adaptive_max = min(base_max_chunks + 8, 30) # 增加量更大，上限提高到30
            # logging.debug(f"帧时间非常低 ({avg_frame_time:.4f}s), 大幅增加加载量至 {adaptive_max}")
            return adaptive_max
        elif avg_frame_time < self.target_frame_time * 0.8:
            # 积极增加，提高上限
            adaptive_max = min(base_max_chunks + 4, 20) # 增加量较大，上限提高到20
            # logging.debug(f"帧时间低 ({avg_frame_time:.4f}s), 增加加载量至 {adaptive_max}")
            return adaptive_max
        # 如果帧时间略高于目标，维持基础值或略微减少
        elif avg_frame_time > self.target_frame_time * 1.1:
             # 保守地减少，确保至少加载1个
            adaptive_max = max(base_max_chunks - 1, 1) # 基于配置值减少
            # logging.debug(f"帧时间高 ({avg_frame_time:.4f}s), 减少加载量至 {adaptive_max}")
            return adaptive_max
