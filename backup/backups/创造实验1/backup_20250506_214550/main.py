...
# 性能优化模块
from frustum_culling import frustum_culling_manager, get_visible_blocks  # 视锥体剔除
from lod_system import lod_manager, process_blocks_lod  # LOD系统
from performance_optimizer import performance_optimizer, handle_optimization_hotkeys  # 性能优化管理器
...
# 区块加载参数
MAX_CHUNKS_PER_UPDATE = 5  # 每次更新最大加载区块数
CHUNK_LOAD_INTERVAL = 0.05  # 区块加载间隔时间
...
chunk_executor = ThreadPoolExecutor(max_workers=num_cores * 2)
...
# 包含游戏中所有方块的侧面纹理
Block_side_textures = [
    load_texture('assets/grass.png'),  # 草地侧面
    load_texture('assets/stone.png'), # 石头侧面
    load_texture('assets/dirt.png'),  # 泥土侧面
    load_texture('assets/bed.png')    # 床侧面
]
...
# 空间网格配置
GRID_CELL_SIZE = 16 # 极度增大网格单元大小，减少网格数量，提高性能
...
class SpatialGrid:
    ...
    def get_visible_blocks(self, position, radius=1):
        """获取视锥体内可见的方块"""
        # 使用性能优化管理器获取可见方块
        return performance_optimizer.get_visible_blocks(self, position, radius)
...
RENDER_DISTANCE = 1  # 保持较低的渲染距离以提高性能
# MAX_CHUNKS_PER_UPDATE: 每帧最大生成区块数量
MAX_CHUNKS_PER_UPDATE = 1  # 保持每帧生成的区块数量为1
# UPDATE_INTERVAL: 区块更新间隔(秒)
UPDATE_INTERVAL = 0.5  # 进一步增加更新间隔以减少CPU负担
# CHUNK_UNLOAD_DISTANCE: 区块卸载距离(以区块为单位)
CHUNK_UNLOAD_DISTANCE = 3  # 更积极地卸载远处区块
...
# 创建粒子对象池
MAX_PARTICLES = 50  # 减少最大粒子数量以降低内存使用
...
class Block(Button):
    ...
    def input(self, key):
        ...
        # 只处理鼠标按键事件，忽略其他输入事件
        if not (key == 'right mouse down' or key == 'left mouse down'):
            return
...
class Chunk:
    ...
    def _finalize_chunk_creation(self, blocks_data):
        ...
        # 在所有创建逻辑（包括可能的 invoke 延迟调用）之后标记为已生成
        # 注意：如果 create_blocks 使用了 invoke，这里标记可能稍微提前，
        # 但比在 create_blocks 内部标记更安全，因为它至少在 finalize 逻辑结束时执行。
        # 理想情况下，需要追踪所有 invoke 调用完成，但这会增加复杂性。
    ...
    def generate(self):
        ...
        if player and hasattr(player, 'position'):
            ...
            # 扩大下方区块的检测范围，确保玩家周围和下方的区块都能被同步生成
            if ((abs(player_x - chunk_center_x) < CHUNK_SIZE * 2 and 
                abs(player_z - chunk_center_z) < CHUNK_SIZE * 2 and 
                player_y - 30 < 0) or  # 玩家下方30格内的区块
                (abs(player_x - chunk_center_x) < CHUNK_SIZE * 1.5 and 
                abs(player_z - chunk_center_z) < CHUNK_SIZE * 1.5 and 
                player_y - 15 < 0)):  # 玩家下方15格内的区块
        ...
        if is_below_player:
            try:
                ...
                # 立即处理区块创建
                self._finalize_chunk_creation(blocks_data)
                return
        ...
        def on_done(future):
            try:
                ...
                # 移除延迟，尽快处理
                invoke(self._finalize_chunk_creation, blocks_data, delay=0)
            except Exception as e:
    ...
    def _create_batch(self, batch_data):
        ...
        # 在锁外批量添加到空间网格
        for block in blocks_added_to_grid:
            spatial_grid.add_block(block)
    ...
    def generate_terrain_data(self):
        ...
        for x in range(CHUNK_SIZE):
            for z in range(CHUNK_SIZE):
                ...
                # 批量添加方块 - 一次性构建所有方块数据
                # 顶层草方块
                blocks_data.append((Vec3(wx, height, wz), 0))
                
                # 批量添加泥土层（顶层-1到顶层-3）
                dirt_bottom = max(height-3, -4)
                for y in range(height-1, dirt_bottom, -1):
                    blocks_data.append((Vec3(wx, y, wz), 2))
                
                # 批量添加石头层（紧接着泥土层，一直到底部）
                stone_top = dirt_bottom - 1  # 石头层从泥土层下方开始
                stone_bottom = -10  # 石头层底部深度
                if stone_top > stone_bottom:
                    for y in range(stone_top, stone_bottom, -1):
                        blocks_data.append((Vec3(wx, y, wz), 1))
                
                # 最底层基岩
                blocks_data.append((Vec3(wx, -3, wz), 3))
        ...
        for x in range(CHUNK_SIZE):
            for z in range(CHUNK_SIZE):
                # 大幅降低树木生成概率，从5%降低到1%
                if heights[(x, z)] > 5 and random() < 0.01:  # 降低单个位置生成树的概率到1%
                    tree_candidates.append((x, z, heights[(x, z)]))
    ...
    def add_tree(self, blocks_data, wx, wz, base_height):
        ...
        for dx in range(-3, 4):
            for dz in range(-3, 4):
                ...
                if (abs(dx) == 3 or abs(dz) == 3) and random() < 0.3:
    ...
    for dx in range(-2, 3):
        for dy in range(-1, 2):
            for dz in range(-2, 3):
                ...
                # 距离越远，生成叶子的概率越低
                if distance <= 3 and random() < (1.0 - distance/4):
                    leaf_pos = Vec3(
                        base_pos.x + dx,
...
def update_chunks():
    ...
    try:
        ...
        # 检查玩家下方是否有区块，如果没有则立即生成
        # 这是防止玩家掉落的关键优化
        # 增加检查深度，确保玩家下方有足够的区块
        for depth in range(1, 15, 5):  # 检查玩家下方1、6、11格的区块
            player_below_chunk = get_chunk_position(Vec3(player.position.x, player.position.y - depth, player.position.z))
            if player_below_chunk not in loaded_chunks:
                try:
                    # 立即生成玩家下方区块，优先级最高
                    chunk = Chunk(player_below_chunk)
                    # 强制同步生成，不使用异步
                    chunk.generate()
                    loaded_chunks[player_below_chunk] = chunk
                    # 强制立即生成，不延迟
                    chunk.is_generated = True
                    
                    # 如果是最近的下方区块，创建临时安全平台防止掉落
                    if depth == 1:
                        # 在玩家正下方创建临时安全平台
                        safe_y = floor(player.position.y) - 2  # 玩家脚下2格
                        for dx in range(-1, 2):
                            for dz in range(-1, 2):
                                safe_pos = Vec3(player.position.x + dx, safe_y, player.position.z + dz)
                                safe_chunk_pos = get_chunk_position(safe_pos)
                                # 确保安全平台所在区块已加载
                                if safe_chunk_pos not in loaded_chunks:
                                    safe_chunk = Chunk(safe_chunk_pos)
                                    safe_chunk.generate()
                                    loaded_chunks[safe_chunk_pos] = safe_chunk
                                    safe_chunk.is_generated = True
                                
                                # 检查位置是否已有方块
                                block_exists = False
                                for block in loaded_chunks[safe_chunk_pos].blocks:
                                    if str(block.position) == str(safe_pos):
                                        block_exists = True
                                        break
                                
                                # 如果没有方块，创建临时安全方块
                                if not block_exists:
                                    new_block = Block(position=safe_pos, id=1)  # 使用石头方块
                                    loaded_chunks[safe_chunk_pos].blocks.append(new_block)
                except Exception as e:
                    print(f"Error generating chunk below player at depth {depth}: {e}")
...
def update():
    global hand, frame_count, last_time, fps, player
    ...
    try:
        ...
        if current_state == GameState.PLAYING and player:
            try:
                ...
                # 检查玩家下方区块是否已加载，如果未加载则禁用重力
                if player and hasattr(player, 'position'):
                    # 检查多个深度的下方区块
                    below_chunks_loaded = True
                    for depth in range(1, 4):  # 检查3个深度级别
                        below_pos = Vec3(player.position.x, player.position.y - depth, player.position.z)
                        below_chunk = get_chunk_position(below_pos)
                        
                        # 如果任何一个下方区块未加载，标记为未加载
                        if below_chunk not in loaded_chunks:
                            below_chunks_loaded = False
                            break
                    
                    # 根据下方区块加载状态调整重力
                    if not below_chunks_loaded:
                        # 如果下方区块未加载，完全禁用重力
                        player.gravity = 0
                        # 重置当前重力值，确保下次从0开始平滑过渡
...
def update_performance_stats():
    ...
    stats_text += f"渲染距离: {performance_optimizer.render_distance_base}\n"
